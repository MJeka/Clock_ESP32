...
void setupOTA() {
    ...
    ArduinoOTA.onStart([]() {
        String type;
        if (ArduinoOTA.getCommand() == U_FLASH) type = "sketch";   // Обновление программы (кода)
        else type = "filesystem";                                 // Обновление файловой системы
    ...
    ArduinoOTA.onError([](ota_error_t error) {
        ...
        if      (error == OTA_AUTH_ERROR)    logInfo("Ошибка: Отказ в авторизации");
        else if (error == OTA_BEGIN_ERROR)   logInfo("Ошибка: Сбой инициализации");
        else if (error == OTA_CONNECT_ERROR) logInfo("Ошибка: Сбой соединения");
        else if (error == OTA_RECEIVE_ERROR) logInfo("Ошибка: Ошибка приема данных");
        else if (error == OTA_END_ERROR)     logInfo("Ошибка: Сбой завершения");
    });
...
String scanNetworks() {
    ...
    String list = "";
    for (int i = 0; i < n; ++i) {
        list += "<option value='" + WiFi.SSID(i) + "'>" + WiFi.SSID(i) + " (" + String(WiFi.RSSI(i)) + " dBm)</option>";
    }
    return list;
}
...
void handleSaveBrightness() {
    if (server.hasArg("d_br")) {
        ...
        preferences.end();
        logInfo("Brightness settings saved");
...
void setupWebHandlers() {
    server.on("/", HTTP_GET, []() {
        server.send(200, "text/html", getIndexPage(scanNetworks(), ssid, dayBrightness, nightBrightness, nightStartHour, nightEndHour));
    });
    ...
    server.on("/save_bright", HTTP_POST, handleSaveBrightness);
...
void startConfigMode() {
    ...
    server.begin();
    update_screen_status(("НАСТРОЙКА\nСеть: " + apName + "\nIP: 192.168.4.1").c_str());
    while (true) {
